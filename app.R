  ...
  output$averagePositionDiff <- renderText({
    if (input$viewSelect == "Town Hall Difference Table" && input$playerTag != "") {
      ...
      if (!is.na(average_position_difference)) {
        paste("Average attack difference on map location for", input$playerTag, ":", average_position_difference, "(Negative values indicate the player attacks above their map position while positive values indicate they attack below their map position.)")
      } else {
